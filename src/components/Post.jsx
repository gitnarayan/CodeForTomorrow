...
const Post = () => {
  ...
  const handleRemovePost = (id) => {
    ...
    let startingIndex = page * 6 - 6;
    let endingIndex = page * 6;
    let limitPosts = updatedPosts.slice(startingIndex, endingIndex);
    setPagePosts(limitPosts);

    // If less than 6 posts on current page, shift from next page
    if (limitPosts.length < 6 && endingIndex < updatedPosts.length) {
      let newLimitPosts = updatedPosts.slice(startingIndex, endingIndex + 1);
      setPagePosts(newLimitPosts);
    }
  };
        ...
        <div className="row w-100">
          {pagePosts.map((post) => (
            <div className="col-md-4 mb-4" key={post.id}>
              <Card object={post} onRemove={() => handleRemovePost(post.id)} />
            </div>
          ))}
        </div>
